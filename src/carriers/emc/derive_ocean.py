"""
derive_ocean.py — EMC v3: derive port-to-port (ocean-only) schedules from inland scrapes.
...
the port:
    ...
    ocean.last_cy = port_of_discharge ;  ocean.eta = ocean.pod_eta = inland.pod_eta
    ocean.transit = (pod_eta - etd).days        # ocean leg only, rail dropped

No estimation. Many inland yards share the same ocean sailing, so derived
schedules are deduped per (origin, port).
...
Run:  python derive_ocean.py   (stdlib only, no scrape; standalone = derive + push)
"""
...
def derive_ocean_schedule(s):
    """Truncate an EMC inland schedule to its ocean leg (port-to-port)."""
    ...
    return {
        "id": _hash16("EMC", P, etd, port_eta, s.get("mother_vessel"),
                      tuple(s.get("vessel_sequence") or [])),
        "port_of_discharge": P,
        ...
        "rail_transit_days": 0,
        "intermodal_legs": [],                     # rail dropped
        "intermodal_transit_days": None,
...
def derive_ocean(canonical_dir=CANONICAL_DIR, ocean_dir=OCEAN_DIR, ports=None):
    ...
    for p in ocean_dir.glob("EMC_*.json"):
        p.unlink()
    ...
    for (origin, P), g in groups.items():
        ...
        p5 = (P or "").replace(" ", "")[:5]
        out = _unique_path(ocean_dir / f"EMC_{o5}_{p5}_{ts}.json")
...
def _ingest_ocean():
    ...
    try:
        ...
        ingest_new_canonicals("EMC", canonical_dir=OCEAN_DIR,
