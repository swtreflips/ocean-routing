"""
derive_ocean.py — OOCL v3: derive port-to-port (ocean-only) schedules from inland scrapes.
...
the port (eta = pod_eta, rail/barge/truck dropped). No estimation. Deduped per
ocean sailing.

NOTE on Long Beach / Tacoma: OOCL's yard set (coverage) contains only Los Angeles
and Seattle. utils.normalize_pod keeps Los Angeles/Long Beach and Seattle/Tacoma
DISTINCT, so any Long Beach or Tacoma services that surface as a
`port_of_discharge` here derive into their own (distinct) canonicals — no merge,
no duplicate. Those ports are never queried separately (not in coverage), i.e.
derive-only (like HPL Long Beach).
...
def derive_ocean_schedule(s):
    """Truncate an OOCL inland schedule to its ocean leg (port-to-port)."""
    ...
    return {
        "id": _hash16("OOCL", P, etd, port_eta, s.get("mother_vessel"),
                      tuple(s.get("vessel_sequence") or [])),
        "port_of_discharge": P,
...
def derive_ocean(canonical_dir=CANONICAL_DIR, ocean_dir=OCEAN_DIR, ports=None):
    ...
    for p in ocean_dir.glob("OOCL_*.json"):
        p.unlink()

    groups = {}
    n_files = n_in = n_derived = n_dup = 0
    for fp in sorted(canonical_dir.glob("OOCL_*.json")):
    ...
    for (origin, P), g in groups.items():
        ...
        p5 = (P or "").replace(" ", "")[:5]
        out = _unique_path(ocean_dir / f"OOCL_{o5}_{p5}_{ts}.json")
...
def _ingest_ocean():
    ...
    try:
        ...
        ingest_new_canonicals("OOCL", canonical_dir=OCEAN_DIR,
