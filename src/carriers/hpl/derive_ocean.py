"""
derive_ocean.py — HPL v3: derive port-to-port (ocean-only) schedules from inland scrapes.
...
the port (eta = pod_eta, rail dropped). No estimation. Deduped per ocean sailing.

NOTE on Long Beach: HPL returns Long Beach services inside the Los Angeles query
and normalize_pod keeps the two DISTINCT. So Long Beach voyages that show up as a
`port_of_discharge` here derive into their own (distinct) Long Beach canonicals —
no merge, no duplicate. Long Beach is never queried separately (not in coverage).
...
def derive_ocean_schedule(s):
    """Truncate an HPL inland schedule to its ocean leg (port-to-port)."""
    ...
    return {
        "id": _hash16("HPL", P, etd, port_eta, s.get("mother_vessel"),
                      tuple(s.get("vessel_sequence") or [])),
        "port_of_discharge": P,
...
def derive_ocean(canonical_dir=CANONICAL_DIR, ocean_dir=OCEAN_DIR, ports=None):
    ...
    for p in ocean_dir.glob("HPL_*.json"):
        p.unlink()

    groups = {}
    n_files = n_in = n_derived = n_dup = 0
    for fp in sorted(canonical_dir.glob("HPL_*.json")):
    ...
    for (origin, P), g in groups.items():
        ...
        p5 = (P or "").replace(" ", "")[:5]
        out = _unique_path(ocean_dir / f"HPL_{o5}_{p5}_{ts}.json")
...
def _ingest_ocean():
    ...
    try:
        ...
        ingest_new_canonicals("HPL", canonical_dir=OCEAN_DIR,
