"""
derive_ocean.py — HMM v3: derive port-to-port (ocean-only) schedules from inland scrapes.
...
def derive_ocean_schedule(s):
    """Truncate an HMM inland schedule to its ocean leg (port-to-port)."""
    ...
    return {
        "id": _hash16("HMM", P, etd, port_eta, s.get("mother_vessel"),
                      tuple(s.get("vessel_sequence") or [])),
        "port_of_discharge": P,
...
def derive_ocean(canonical_dir=CANONICAL_DIR, ocean_dir=OCEAN_DIR, ports=None):
    ...
    for p in ocean_dir.glob("HMM_*.json"):
        p.unlink()
    ...
    for (origin, P), g in groups.items():
        ...
        p5 = (P or "").replace(" ", "")[:5]
        out = _unique_path(ocean_dir / f"HMM_{o5}_{p5}_{ts}.json")
...
def _ingest_ocean():
    ...
    try:
        ...
        ingest_new_canonicals("HMM", canonical_dir=OCEAN_DIR,
