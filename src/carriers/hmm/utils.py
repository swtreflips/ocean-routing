...
with open(CARRIER_DIR / "assets" / "hmm_cities.json", "r", encoding="utf-8") as f:
    hmm_cities = json.load(f)

# Build case-insensitive lookup: lowercased city name -> code1
_hmm_code_lookup = {}
for _key, _entries in hmm_cities.items():
    try:
        _hmm_code_lookup[_key.lower()] = _entries[0]["codes"]["code1"]
    except (IndexError, KeyError):
        pass


def get_hmm_code(city_name):
    """Return HMM code1 for a city name, or None if not found."""
    if not city_name:
        return None
    return _hmm_code_lookup.get(city_name.strip().lower())
...
def build_voronoi_lookup(quotes_progress, locations, gdf_voronoi):
    ...
    """
    # Step 1: Deduplicate destinations (drop NaN to avoid float64/str merge errors)
    unique_dest = (
        quotes_progress[["Final Destination"]]
        .dropna(subset=["Final Destination"])
        .drop_duplicates()
    )
    unique_dest["Final Destination"] = unique_dest["Final Destination"].astype(str)

    # Step 2: Attach coordinates from locations (coerce key to str for safe merge)
...
def get_locations(city_name):
    city_entry = hmm_cities.get(city_name)
    if not city_entry:
        return []
...
def normalize_pod(pod):
    ...
    1. Lookup against PORT_NAMES (case-insensitive; accepts bare 'city' or full 'city, st').
    2. Fallback: if the value has a 2-letter state suffix, title-case the city and
       uppercase the state code ('PORT EVERGLADES, FL' -> 'Port Everglades, FL').
...
# HMM mode classification. Water-borne modes (VESSEL, FEEDER, BARGE) count as ocean
# per the structurePhilosophy rule. TRUCK / RAIL are ground.
_HMM_OCEAN_MODES = {"VESSEL", "FEEDER DIRECT", "FEEDER", "BARGE DIRECT", "BARGE"}
_HMM_MODE_LABELS = {
    "FEEDER DIRECT": "FEEDER",
    "FEEDER": "FEEDER",
    "BARGE DIRECT": "BARGE",
    "BARGE": "BARGE",
    "TRUCK DIRECT": "TRUCK",
...
def _hmm_leg_label(leg):
    """For VESSEL legs return 'VesselName/Voyage'; otherwise return short mode label."""
    mode = (leg.get("trsPtnModeNm") or "").strip().upper()
    if mode == "VESSEL":
        vname = (leg.get("vslNm") or "").strip()
        voy = (leg.get("vyagNo") or leg.get("voyageNumber") or "").strip()
        return f"{vname}/{voy}" if voy else (vname or "VESSEL")
    # FEEDER DIRECT / BARGE DIRECT / TRUCK DIRECT / RAIL DIRECT etc.
    return _HMM_MODE_LABELS.get(mode, mode or "")


def _hmm_legs(schedule):
    """
    Normalize an HMM schedule's transit legs into:
      [{pol, pod, label, is_ocean, etd, eta}, ...]

    HMM's leg field naming is unusual:
      - arvlStDt  = "arrival at start of leg" = ETD (when cargo arrives to start the leg)
      - dpartFnshDt = "departure at finish of leg" = ETA (arrival at destination)
    """
    out = []
    for leg in schedule.get("transit", []) or []:
        mode = (leg.get("trsPtnModeNm") or "").strip().upper()
        out.append({
            "pol": leg.get("orgLocNm") or leg.get("dprtrLocNm"),
            "pod": leg.get("destnLocNm"),
            "label": _hmm_leg_label(leg),
            "is_ocean": mode in _HMM_OCEAN_MODES,
            "etd": leg.get("arvlStDt"),
            "eta": leg.get("dpartFnshDt"),
        })
    return out


def _hmm_legs_summary(legs):
    """
...
def build_schedule_rows(file_path):
    """
    Read a wrapped HMM JSON and return flat CSV rows (one per schedule).
    Applies the ocean-only TS rule. BARGE/FEEDER count as ocean.
    """
    ...
    for sched in data.get("schedules", []) or []:
        ...
        if not legs:
            continue
        transport_type, mother_vessel, ts_ports, ts_vessels, _, _ = _hmm_legs_summary(legs)
        ...
        cutoff = _iso_date_or_none(sched.get("gctCtofDt"))

        rows.append({
            **wrap,
            "Port of Discharge": normalize_pod(pod) or "",
            "ETD": etd or "",
            "ETA": eta or "",
            "POD ETA": pod_eta or "",
...
def build_canonical_record(file_path):
    """
    Read a wrapped HMM JSON and return ONE canonical record per query, with
    ...
    for sched in data.get("schedules", []) or []:
        ...
        if not legs:
            continue
        transport_type, mother_vessel, ts_ports, ts_vessels, route_ports, vessel_sequence = _hmm_legs_summary(legs)
