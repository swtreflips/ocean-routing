"""
derive_ocean.py — ONE v3: derive port-to-port (ocean-only) schedules from inland scrapes.
...
NOTE on Long Beach / Seattle: ONE's yard set (coverage) contains only Los Angeles
...
def derive_ocean_schedule(s):
    """Truncate a ONE inland schedule to its ocean leg (port-to-port)."""
    ...
    return {
        "id": _hash16("ONE", P, etd, port_eta, s.get("mother_vessel"),
                      tuple(s.get("vessel_sequence") or [])),
        "port_of_discharge": P,
...
def derive_ocean(canonical_dir=CANONICAL_DIR, ocean_dir=OCEAN_DIR, ports=None):
    ...
    for p in ocean_dir.glob("ONE_*.json"):
        p.unlink()

    groups = {}
    n_files = n_in = n_derived = n_dup = 0
    for fp in sorted(canonical_dir.glob("ONE_*.json")):
    ...
    for (origin, P), g in groups.items():
        ...
        p5 = (P or "").replace(" ", "")[:5]
        out = _unique_path(ocean_dir / f"ONE_{o5}_{p5}_{ts}.json")
...
def _ingest_ocean():
    ...
    try:
        ...
        ingest_new_canonicals("ONE", canonical_dir=OCEAN_DIR,
